...
This function converts the Excalidraw Element Skeleton to excalidraw elements which could be then rendered on the canvas. Hence calling this function is necessary before passing it to APIs like [`initialData`](https://excalidraw.issirmax.mx/docs/@excalidraw/excalidraw/api/props/initialdata), [`updateScene`](https://excalidraw.issirmax.mx/docs/@excalidraw/excalidraw/api/props/excalidraw-api#updatescene) if you are using the Skeleton API
